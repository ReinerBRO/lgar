...
LOG_FILE="logs/cure_v3_freeze_gate_${TIMESTAMP}_${SLURM_JOB_ID}.log"
exec > "${LOG_FILE}" 2>&1
...
OUTPUT_ROOT=${OUTPUT_ROOT:-/data/user/xnie012/pythonprojects/lgar/runs/cure_v3_freeze_gate_${TIMESTAMP}}
REPORT_DIR=${REPORT_DIR:-/data/user/xnie012/pythonprojects/lgar/reports/cure_v3_freeze_gate_${TIMESTAMP}}
...
echo "=== CURE-v3 freeze-base full-path gate ==="
...
run_variant() {
  ...
  echo "=== train ${name} ==="
  echo "ablate=${ablate} lambda_full_hu=${lambda_full_hu} adapter_lr=${adapter_lr}"
  date
...
print(json.dumps({"event": "cure_v3_freeze_gate_summary", **summary}, sort_keys=True), flush=True)
PY
}

# Remove base-continuation drift: CE checkpoint is the fixed base, and only
# retrieval-head adapters train on full-path CE + high-utility CE.
run_variant "cure_v3_freeze_top24_fullhu05_alr1e4" "${TOP24}" "0.5" "1e-4"
run_variant "cure_v3_freeze_top24_fullhu10_alr1e4" "${TOP24}" "1.0" "1e-4"

echo "=== CURE-v3 freeze-base gate done ==="
date
echo "report_dir=${REPORT_DIR}"
