...
LOG_FILE="./logs/pilot_adapter_ce_${TIMESTAMP}_${SLURM_JOB_ID}.log"
exec > "$LOG_FILE" 2>&1
...
OUTPUT_DIR=/data/user/xnie012/pythonprojects/lgar/runs/pilot_adapter_ce_${TIMESTAMP}

echo "=== Pilot B: Adapter-CE (4 GPU, 50M global, top-6 heads) ==="
date; nvidia-smi --query-gpu=index,memory.used --format=csv || true

torchrun --standalone --nproc_per_node=4 -m curcpt.train \
  --run-name pilot_adapter_ce \
  --method adapter_ce \
...
date; echo "=== done ==="
cat "${OUTPUT_DIR}/runs/cure/pilot_adapter_ce/summary.json" 2>/dev/null
