...
LOG_FILE="logs/cure_v4_freeze_fraction_gate_${TIMESTAMP}_${SLURM_JOB_ID}.log"
exec > "${LOG_FILE}" 2>&1
...
OUTPUT_ROOT=${OUTPUT_ROOT:-/data/user/xnie012/pythonprojects/lgar/runs/cure_v4_freeze_fraction_gate_${TIMESTAMP}}
REPORT_DIR=${REPORT_DIR:-/data/user/xnie012/pythonprojects/lgar/reports/cure_v4_freeze_fraction_gate_${TIMESTAMP}}
...
echo "=== CURE-v4 freeze-base utility-fraction gate ==="
...
run_variant() {
  local frac="$1"
  local lambda_full_hu="$2"
  local name="cure_v4_freeze_top24_frac${frac/./}_fullhu${lambda_full_hu/./}_alr1e4"
  ...
  echo "=== train ${name} ==="
  echo "frac=${frac} lambda_full_hu=${lambda_full_hu}"
  date
  torchrun --standalone --nproc_per_node=4 -m curcpt.train \
    ...
    --utility-top-fraction-training "${frac}" \
    --lora-rank 16 \
    --lora-alpha 32.0 \
    --eval-interval 25 \
    --seed 1337 \
...
print(json.dumps({
    "event": "cure_v4_fraction_gate_summary",
    "variant": name,
...
run_variant "0.20" "1.0"
run_variant "0.30" "1.0"

echo "=== CURE-v4 fraction gate done ==="
date
echo "report_dir=${REPORT_DIR}"
