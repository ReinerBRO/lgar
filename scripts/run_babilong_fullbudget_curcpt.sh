...
LOG_FILE="logs/babilong_fullbudget_${TIMESTAMP}_${SLURM_JOB_ID}.log"
exec > "${LOG_FILE}" 2>&1
...
export BABILONG_ROOT=/data/user/xnie012/cache/data/babilong
...
torchrun --standalone --nproc_per_node=4 -m curcpt.eval_long_context_public \
  ...
  --benchmark babilong \
  --tasks qa2_8k,qa2_8k_perk_paper \
  --output "${REPORT_DIR}/babilong_fullbudget.json" \
...
echo "=== BABILong fullbudget eval done ==="
